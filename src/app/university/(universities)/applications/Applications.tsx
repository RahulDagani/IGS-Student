...
const FilterModal: React.FC<FilterModalProps> = ({
  ...
  const handleApply = () => {
    const filters: FilterOptions = {
      ...
      studylevel : selectedCountry,
      discipline: selectedDiscipline,
      country:selectedStudyLevel
    };
    onApply(filters);
    onClose();
  };
    ...
    <div className="w-full bg-white dark:bg-gray-800 rounded-2xl shadow-md p-5 border border-gray-100 dark:border-gray-700">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-800 dark:text-white leading-snug mb-2">
              {application.course}
            </h2>
      ...
      <div className="border-t border-gray-100 dark:border-gray-700 mt-4 pt-4 space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
          ...
          <span>
            <strong className="font-semibold text-gray-800 dark:text-white">studylevel:</strong>{" "}
...
export default function ApplicationsTable() {
  ...
  const countries = useMemo(() => {
    return Array.from(new Set(tableData.map(app => app.country)));
  }, []);

  const disciplines = useMemo(() => {
    return Array.from(new Set(tableData.map(app => app.discipline)));
  }, []);

  const studylevels = useMemo(() => {
    return Array.from(new Set(tableData.map(app => app.studylevel)));
  }, []);
...
const filteredAndSortedData = useMemo(() => {
  const filtered = tableData.filter((application) => {
    ...
    const matchesCountry = filters.country === "all" || application.country === filters.country;
    const matchesDiscipline = filters.discipline === "all" || application.discipline === filters.discipline;
    const matchesStudyLevel = filters.studylevel === "all" || application.studylevel === filters.studylevel;

    
    return matchesCountry && matchesDiscipline && matchesStudyLevel;
  });
  ...
  const hasActiveFilters = filters.country !== "all" || filters.discipline !== "all" || filters.studylevel !== "all";
