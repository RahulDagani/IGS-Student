...
// New API Response Interfaces for Courses
interface Course {
  id: number;
  tenant_id: number;
...
export default function StudentPrograms() {
  ...
  const fetchCourses = useCallback(async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      const url = `${BASE_URL}/student/courses/list${queryString ? `?${queryString}` : ''}`;
  ...
  const handleConfirmApplication = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/student/application/add`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Browse and apply to programs that match your interests.
          </p>
        </div>
      ...
      <div className="flex justify-between items-center text-sm text-gray-500 dark:text-gray-400">
        <div>
          Showing {apiResponse?.data?.length || 0} of {apiResponse?.total || 0} programs
          {apiResponse && (
            <span className="ml-2">
        ...
        {apiResponse && apiResponse.totalPages > 1 && (
          <div className="text-sm">
            {currentPage < apiResponse.totalPages ? 'Scroll to load more' : 'All programs loaded'}
          </div>
        )}
      </div>
