...
interface SlabApiResponse {
  ...
  data: {
    ...
    min_students: number;
    max_students: number;
    commission_percentage: string; // Comes as string from API
    description: string;
    is_active: number;
    created_at: string;
    updated_at: string;
...
export default function EditSlab() {
  const router = useRouter();
  const params = useParams();
  const slabId = params.id; // Get slab ID from URL
  ...
  // Fetch slab data on component mount
  useEffect(() => {
    const fetchSlabData = async () => {
      ...
      try {
        ...
        const response = await fetch(`${BASE_URL}/tenant/slabs/${slabId}`, {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch slab data: ${response.status}`);
        }

        const result: SlabApiResponse = await response.json();

        if (result.success && result.data) {
          const slab = result.data;
          // Convert API data to form state (numbers to strings for inputs)
          setFormData({
            slab_name: slab.slab_name,
            min_students: slab.min_students.toString(),
            max_students: slab.max_students.toString(),
        ...
        console.error('Error fetching slab:', err);
        setFetchError(err instanceof Error ? err.message : 'Failed to load slab data');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Slab updated successfully:", result);
      ...
      setError(error instanceof Error ? error.message : "Failed to update slab");
    } finally {
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Loading slab data...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="p-8 text-center">
          <div className="rounded-lg bg-red-50 p-4 dark:bg-red-900/20 mb-4">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-800 dark:text-red-200">Error Loading Slab</h3>
                <p className="text-sm text-red-700 dark:text-red-300 mt-1">{fetchError}</p>
              </div>
            </div>
          </div>
          <button
            onClick={() => router.push('/admin/partners/slabs')}
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800"
          >
            Back to Slabs List
          </button>
        </div>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="px-5 py-4 sm:px-6 sm:py-5">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Update the commission slab details.
        </p>
      </div>
